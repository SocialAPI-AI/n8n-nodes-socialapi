...
export async function execute(this: IExecuteFunctions, operation: string, i: number): Promise<INodeExecutionData[]> {
	...
	switch (operation) {
		...
		case 'create': return wrap(await apiRequest.call(this, 'POST', '/brands', { name: this.getNodeParameter('name', i) }));
		case 'update': {
			const id = this.getNodeParameter('brandId', i) as string;
			return wrap(await apiRequest.call(this, 'PATCH', `/brands/${id}`, { name: this.getNodeParameter('name', i) }));
		}
		case 'delete': {
			const id = this.getNodeParameter('brandId', i) as string;
			return wrap(await apiRequest.call(this, 'DELETE', `/brands/${id}`));
		}
		default: throw new Error(`Unknown brand operation: ${operation}`);
	}
}
