...
export async function execute(
	...
	switch (operation) {
		case 'get':
			return wrap(await apiRequest.call(this, 'GET', `/inbox/conversations/${id}`));
		case 'listMessages':
			return list(`/inbox/conversations/${id}/messages`, {});
		case 'send':
			return wrap(
				await apiRequest.call(this, 'POST', `/inbox/conversations/${id}/messages`, {
					text: this.getNodeParameter('text', i),
				}),
			);
		case 'markRead':
			return wrap(await apiRequest.call(this, 'POST', `/inbox/conversations/${id}/read`));
		case 'update':
			return wrap(
				await apiRequest.call(this, 'PATCH', `/inbox/conversations/${id}`, {
					is_read: this.getNodeParameter('is_read', i),
				}),
			);
		default:
