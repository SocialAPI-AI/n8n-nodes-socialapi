...
const boundApiRequest: ApiRequestFn = (ctx, method, path, body, qs) =>
	apiRequest.call(ctx, method, path, body, qs);
...
export async function runUpload(
	ctx: IExecuteFunctions,
	i: number,
	req: ApiRequestFn = boundApiRequest,
): Promise<IDataObject> {
	...
	// 1. Presign.
	const info = await req(ctx, 'GET', '/media/upload-url', undefined, {
...
export async function execute(
	...
	switch (operation) {
		case 'upload':
			return wrap(await runUpload(this, i));
		case 'getAll': {
		...
		case 'storage':
			return wrap(await apiRequest.call(this, 'GET', '/media/storage'));
		case 'delete': {
