...
export async function execute(
	...
	if (operation === 'create' || operation === 'validate') {
		...
		const targetsRaw = (this.getNodeParameter('targets', i) as IDataObject).target as
			| IDataObject[]
			| undefined;
		...
		if (operation === 'create') {
			...
			// validate: include media so media-specific constraints are checked
			const mediaIds = this.getNodeParameter('validateMediaIds', i, '') as string;
			if (mediaIds) body.media_ids = csv(mediaIds);
		}
	...
	if (operation === 'getConstraints')
		return wrap(await apiRequest.call(this, 'GET', '/posts/validate'));

	const postId = this.getNodeParameter('postId', i) as string;
	switch (operation) {
		case 'get':
			return wrap(await apiRequest.call(this, 'GET', `/posts/${postId}`));
		case 'getMetrics':
			return wrap(await apiRequest.call(this, 'GET', `/posts/${postId}/metrics`));
		case 'delete':
			return wrap(await apiRequest.call(this, 'DELETE', `/posts/${postId}`));
		case 'publish':
			return wrap(await apiRequest.call(this, 'POST', `/posts/${postId}/publish`));
		case 'retry':
			return wrap(await apiRequest.call(this, 'POST', `/posts/${postId}/retry`));
		case 'unpublish': {
